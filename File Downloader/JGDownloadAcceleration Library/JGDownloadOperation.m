...
- (instancetype)initWithRequest:(NSURLRequest *)request destinationPath:(NSString *)path allowResume:(BOOL)resume customSize:(long long)customDownloadSize {
    self = [super init];
    if (self) {
        ...
        _append = resume;
         _customDownloadSize = customDownloadSize;
...
- (void)startLoadingAllConnectionsAndOpenOutput {
    NSLog(@"HERE %@:", self.destinationPath);
...
- (void)didReceiveHTTPHeaders:(NSHTTPURLResponse *)response error:(NSError *)error {
    if (_error) {
        ...
        unsigned long long size = _customDownloadSize;//(unsigned long long)[response expectedContentLength];
...
- (void)download:(JGDownload *)download didReadData:(NSData *)data {
    ...
    NSLog(@"DOWNLOADED FILE DATA %@: ", data);
